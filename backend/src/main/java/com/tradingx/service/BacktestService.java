...
public class BacktestService {
    ...
    private final StrategyCompilerCache strategyCompilerCache;

    public BacktestService(StrategyService strategyService,
                           StrategyCompilerCache strategyCompilerCache) {
    ...
    public BacktestResult runBacktest(Long strategyId, List<DailyQuote> quotes) {
        ...
        return runBacktest(strategyEntity, quotes);
    }

    public BacktestResult runBacktest(com.tradingx.model.Strategy strategyEntity, List<DailyQuote> quotes) {
        ...
        if (strategyEntity.getCode() != null && !strategyEntity.getCode().isBlank()) {
            CompiledStrategy compiled = strategyCompilerCache.getOrCompile(
                    strategyEntity.getId(),
                    strategyEntity.getCode(),
                    strategyEntity.getLanguage()
            );
            ta4jStrategy = compiled.create(series);
        } else {
        ...
        log.info("Backtest: strategyId={}, seriesBars={}, closedPositions={}, openPositions={}, totalTrades={}",
                strategyEntity.getId(), series.getBarCount(),
    ...
    private BacktestResult.BacktestSignal createSignal(Trade trade, BarSeries series, String type) {
        ...
        signal.setTradeDate(series.getBar(trade.getIndex()).getEndTime()
