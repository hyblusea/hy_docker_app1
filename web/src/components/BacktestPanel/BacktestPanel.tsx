...
const BacktestPanel = ({ quotes, resetKey, onSignals, onClearSignals }: BacktestPanelProps) => {
  ...
  const subtotalProfitPct = result ? result.total_return : 0
  ...
  const handleExportExcel = useCallback(() => {
    ...
    const rows: Record<string, string | number>[] = trades.map((t, i) => ({
      ...
      买入股数: t.entry.shares,
      买入总金额: t.entry.buy_amount.toFixed(2),
      买入手续费: t.entry.fees.toFixed(2),
      卖出日期: t.exit ? formatTradeDate(t.exit.trade_date) : '-',
      卖出价: t.exit ? t.exit.price.toFixed(2) : '-',
      卖出手续费: t.exit ? t.exit.sell_fees.toFixed(2) : '-',
      盈亏金额: t.exit ? (t.exit.profit >= 0 ? '+' : '') + (t.exit.profit / 10000).toFixed(2) + '万' : '-',
      收益率: t.exit ? (t.exit.profit_pct >= 0 ? '+' : '') + t.exit.profit_pct.toFixed(2) + '%' : '-',
      剩余金额: formatMoney(t.exit ? t.exit.remaining_cash : t.entry.remaining_cash),
    }))
    rows.push({
      序号: '',
      买入日期: '小计',
      ...
      <div className={styles.header} onClick={() => setExpanded(!expanded)}>
        ...
        {result && (
          <span className={styles.headerSummary}>
            [{strategyName}]  收益率: <span className={result.total_return >= 0 ? styles.valUp : styles.valDown}>
              {result.total_return >= 0 ? '+' : ''}{result.total_return.toFixed(2)}%
            </span>
            {', '}收益: <span className={result.profit_loss >= 0 ? styles.valUp : styles.valDown}>
              {result.profit_loss >= 0 ? '+' : ''}{formatMoney(result.profit_loss)}元
            </span>
            {', '}交易: {result.trade_count}次
            {result.open_position_count > 0 && <span style={{ color: '#faad14' }}> (+{result.open_position_count}持仓中)</span>}
          </span>
        )}
      </div>
      ...
      >
        ...
        {trades.length > 0 && (
          <div className={styles.detailSubtotal}>
            ...
            <span>{formatMoney(result?.final_capital ?? 0)}</span>
          </div>
        )}
      </Modal>
    </div>
