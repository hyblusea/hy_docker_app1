...
function generateRuleJava(node: RuleNode, varPrefix: string, counter: { val: number }): { lines: string[], lastVar: string } {
  const lines: string[] = []

  if (node.kind === 'leaf') {
    const def = getRuleDef(node.type)
    if (!def) return { lines, lastVar: '' }
    ...
    if (node.negated) {
      ...
      return { lines, lastVar: negVar }
    }
    return { lines, lastVar: ruleVar }
  }

  const childVars: string[] = []
  for (const child of node.children) {
    const startCount = counter.val
    const childResult = generateRuleJava(child, varPrefix, counter)
    ...
    const lastVar = node.negated && child.negated
      ...
      : child.kind === 'leaf' && !child.negated
        ...
        : childResult.lastVar || `${varPrefix}${counter.val - 1}`
    childVars.push(lastVar)
  }

  if (node.children.length === 0) {
    const emptyVar = `${varPrefix}${counter.val++}`
    lines.push(`        // 空规则(false)`)
    lines.push(`        Rule ${emptyVar} = new BooleanRule(false);`)
    return { lines, lastVar: emptyVar }
  } else if (node.children.length === 1) {
    return { lines, lastVar: childVars[0] }
  } else {
    ...
    return { lines, lastVar: combinedVar }
  }
}
...
export function generateJavaCode(config: VisualStrategyConfig, className: string): string {
  ...
  const entryResult = generateRuleJava(config.entryRule, 'entry', { val: 0 })
  const exitResult = generateRuleJava(config.exitRule, 'exit', { val: 0 })

  const entryVar = entryResult.lastVar || 'entry0'
  const exitVar = exitResult.lastVar || 'exit0'
...
${varLines.join('\n')}
        ...
        // ===== 入场规则 =====
${entryResult.lines.join('\n')}

        // ===== 出场规则 =====
${exitResult.lines.join('\n')}

        return new BaseStrategy(${entryVar}, ${exitVar});
    }
}
`
}
